...
public class XLSXFile {
    ...
    public func parseDocumentPaths() throws -> [String] {
        try parseRelationships().items
            .filter { $0.schemaType == .officeDocument }
            .map { $0.target }
    }

    public func parseThemePaths(forDocumentPath documentPath: String) throws -> [String] {
        try parseDocumentRelationships(path: documentPath).1.items
            .filter { $0.schemaType == .theme }
            .map { $0.target }
    }
    ...
    public func parseThemes() throws -> [Theme] {
        let paths = try parseThemePaths(forDocumentPath: "xl/workbook.xml")

        decoder.keyDecodingStrategy = .useDefaultKeys

        return try paths.map {
            try parseEntry("xl/" + $0, Theme.self)
        }
    }
    ...
    public func parseWorksheetPathsAndNames(
        ...
        try parseDocumentPaths().map {
            ...
            let worksheets = relationships.items.filter { $0.schemaType == .worksheet }
